...
/* Reparto del tiempo despierto: la primera ventana es la más corta y la
   que precede a la noche, la más larga. Suman exactamente las horas que
   el bebé pasa despierto entre el despertar y la hora de dormir. */
function wakeWindows(b) {
  const n = b.naps + 1;
  const awake = (1440 - b.night * 60) - b.dayNap;
  if (n === 1) return [Math.round(awake / 5) * 5];
  const raw = Array.from({ length: n }, (_, i) => 0.82 + 0.46 * (i / (n - 1)));
  const sum = raw.reduce((a, x) => a + x, 0);
  return raw.map((x) => Math.round((awake * x) / sum / 5) * 5);
}

/* El sueño diurno se reparte con las siestas de la mañana más largas
   y la última del día como una siesta corta de recuperación. */
function napLengths(b) {
  const n = b.naps;
  const w = Array.from({ length: n }, (_, i) => 1.35 - 0.75 * (n === 1 ? 0 : i / (n - 1)));
  const sum = w.reduce((a, x) => a + x, 0);
  return w.map((x) => Math.max(20, Math.round((b.dayNap * x) / sum / 5) * 5));
}
...
/* ═══════════════════════════════════════════════════════════════
   ...
   siesta → ventana → siesta … hasta la hora de dormir.
   ═══════════════════════════════════════════════════════════════ */
function plan(now = Date.now()) {
  const b = band();
  const windows = wakeWindows(b);
  const lengths = napLengths(b);
  ...
  if (state.sleepingSince) {
    const expected = lengths[Math.min(index, lengths.length - 1)] * MIN;
    live = { kind: "nap", index, start: state.sleepingSince, end: state.sleepingSince + expected, est: true, live: true };
    blocks.push(live);
    anchor = live.end;
    index++;
  }
  ...
  const upcoming = [];
  for (let i = index; i < b.naps; i++) {
    const start = anchor + windows[Math.min(i, windows.length - 1)] * MIN;
    if (new Date(start).getHours() >= 18) break;      // tan tarde ya no es siesta: es la noche
    const block = { kind: "nap", index: i, start, end: start + lengths[i] * MIN, est: true };
    upcoming.push(block);
    blocks.push(block);
    anchor = block.end;
  }

  /* La noche cierra el día: última ventana desde el final del último
     sueño, acotada a un rango razonable. */
  let bedtime = anchor + windows[windows.length - 1] * MIN;
  ...
  return {
    b, windows, lengths, morningWake, naps, blocks,
    next, live, bedtime,
...
function renderRing() {
  ...
  /* Sin rótulos de hora sobre el anillo: la ventana completa ya se lee
     en la hoja de abajo y aquí solo se pisarían entre sí. */
...
function renderSheet() {
  ...
  const lead = Math.min(30, Math.round(p.windows[Math.min(n.index || 0, p.windows.length - 1)] * 0.25));
  const signsAt = n.start - lead * MIN;
...
function refreshToday() { renderRing(); renderSheet(); }
function refreshAll() {
...
function bandPreview(birth) {
  ...
  const win = wakeWindows(b);
  return `Etapa ${b.label}: ${b.naps} ${b.naps === 1 ? "siesta" : "siestas"}, ventanas de vigilia de ${dur(win[0] * MIN)} a ${dur(win[win.length - 1] * MIN)}.`;
}
